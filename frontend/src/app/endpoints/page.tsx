...
import { Activity, Server, UploadCloud, LogOut, BarChart3, Loader2 } from "lucide-react";
  ...
  const fetchEndpoints = async (userId: string) => {
    try {
      const res = await fetch(
        `https://hookwatch-backend.onrender.com/api/v1/auth/endpoints?user_id=${userId}`,
      );
      if (res.ok) {
      ...
      <main className="flex-1 flex flex-col h-full bg-zinc-950 overflow-y-auto">
        ...
        <div className="p-6 max-w-[1600px]">
          <div className="grid grid-cols-3 gap-4">
            {loading ? (
              <div className="col-span-3 text-zinc-500 text-sm flex items-center justify-center gap-2 p-8">
              ...
              endpoints.map((ep) => (
                <div
                  key={ep.endpoint_id}
                  className="ui-panel p-5 flex flex-col gap-4"
                >
                  ...
                  <div className="flex items-baseline gap-2 mt-2">
                    <span
                      className={`text-2xl font-semibold ${ep.health_score > 90 ? "text-green-500" : ep.health_score > 70 ? "text-amber-500" : "text-red-500"}`}
                    >
                      {ep.health_score}%
                    </span>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-2 pt-4 border-t border-zinc-800/60">
                    ...
                    <div>
                      ...
                      <div
                        className={`font-medium ${ep.failed_requests > 0 ? "text-red-400" : "text-zinc-200"}`}
                      >
