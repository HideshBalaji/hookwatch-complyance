                ...
                try {
                    ...
                    if (statusRes.ok) {
                        ...
                        if (statusData.status === 'processing') {
                            setToast({ message: `Ingesting logs... ${statusData.processed} / ${statusData.total} events`, type: "loading", taskId: parsed.taskId });
                            sessionStorage.setItem('hookwatch_toast', JSON.stringify({ message: `Ingesting logs... ${statusData.processed} / ${statusData.total} events`, type: "loading", taskId: parsed.taskId }));
                        } else if (statusData.status === 'complete') {
                            clearInterval(interval);
                            const successToast: ToastOptions = { message: `Successfully ingested ${statusData.processed} events and ${statusData.attempts_processed} attempts.`, type: "success" };
                            setToast(successToast);
                            sessionStorage.setItem('hookwatch_toast', JSON.stringify(successToast));
                            setTimeout(() => {
                                setToast(null);
                                sessionStorage.removeItem('hookwatch_toast');
                            }, 5000);
                            ...
                            clearInterval(interval);
                            const errToast: ToastOptions = { message: `Upload failed: ${statusData.error}`, type: "error" };
                            setToast(errToast);
                            sessionStorage.setItem('hookwatch_toast', JSON.stringify(errToast));
                            setTimeout(() => {
                                setToast(null);
                                sessionStorage.removeItem('hookwatch_toast');
                            }, 5000);
