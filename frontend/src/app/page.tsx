...
import { Activity, ShieldAlert, GitCommit, Server, UploadCloud, LogOut, CheckCircle2, XCircle, AlertCircle, Clock } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const [analytics, setAnalytics] = useState({ health_score: 100, replays_prevented: 0, active_anomalies: 0 });
    ...
    <div className="flex h-screen bg-zinc-950 text-zinc-300 font-sans text-sm selection:bg-zinc-800 selection:text-zinc-100 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-full bg-zinc-950 overflow-y-auto">
        <header className="px-6 py-4 flex items-center justify-between border-b border-zinc-800/60 sticky top-0 bg-zinc-950 z-10">
          <h2 className="text-lg font-medium text-zinc-100 tracking-tight">Delivery Intelligence</h2>
          <div className="flex items-center gap-2 text-xs font-medium">
             {analytics.health_score > 80 ? (
                 <div className="flex items-center gap-1.5 text-zinc-400 bg-zinc-900 px-2.5 py-1 rounded-full border border-zinc-800"><div className="w-2 h-2 rounded-full bg-green-500"></div> System Stable</div>
             ) : (
                 <div className="flex items-center gap-1.5 text-zinc-400 bg-zinc-900 px-2.5 py-1 rounded-full border border-zinc-800"><div className="w-2 h-2 rounded-full bg-amber-500"></div> Degraded Performance</div>
             )}
          </div>
        </header>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="ui-panel p-4 flex flex-col gap-1">
              ...
              <div className="flex items-baseline gap-2 mt-1">
                <span className="text-2xl font-semibold text-zinc-100">{analytics.health_score}%</span>
                <span className="text-green-500 text-xs font-medium">+2%</span>
              </div>
            </div>
            
            <div className="ui-panel p-4 flex flex-col gap-1">
              <span className="text-zinc-500 text-xs font-medium uppercase tracking-wider">Replays Prevented</span>
              <div className="flex items-baseline gap-2 mt-1">
                <span className="text-2xl font-semibold text-zinc-100">{analytics.replays_prevented.toLocaleString()}</span>
              </div>
            </div>
            
            <div className="ui-panel p-4 flex flex-col gap-1">
              <span className="text-zinc-500 text-xs font-medium uppercase tracking-wider flex items-center gap-1.5">
                  <ShieldAlert className="w-3.5 h-3.5 text-zinc-400" /> Active Anomalies
              </span>
              <div className="flex items-baseline gap-2 mt-1">
                <span className={`text-2xl font-semibold ${analytics.active_anomalies > 0 ? 'text-red-400' : 'text-zinc-100'}`}>{analytics.active_anomalies}</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-12 gap-6 flex-1 min-h-[500px]">
            ...
            <div className="col-span-7 ui-panel flex flex-col overflow-hidden">
              <div className="px-4 py-3 border-b border-zinc-800/60 bg-zinc-900/50 flex justify-between items-center">
                <h3 className="text-sm font-medium text-zinc-200">Delivery Attempts</h3>
                <span className="text-xs text-zinc-500 font-medium">Last 24 hours</span>
              </div>
              
              <div className="flex-1 overflow-y-auto">
                {loadingWebhooks ? (
                  <div className="p-8 text-center text-zinc-500 text-sm">Loading telemetry...</div>
                ) : webhooks.length === 0 ? (
                  <div className="p-8 text-center text-zinc-500 text-sm">No webhook deliveries recorded.</div>
                ) : (
                  <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-zinc-800/60">
                          {webhooks.map((evt) => (
                              ...
                              >
                                  <td className="px-4 py-3 text-xs font-mono text-zinc-400">{evt.event_id}</td>
                                  <td className="px-4 py-3 text-sm text-zinc-200 font-medium">{evt.event_type}</td>
                                  ...
                                  <td className="px-4 py-3 text-xs text-zinc-500 text-right">{new Date(evt.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</td>
                              </tr>
                          ))}
                      </tbody>
                  </table>
            ...
            <div className="col-span-5 ui-panel flex flex-col overflow-hidden bg-zinc-950">
                {!selectedEventId ? (
                    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-zinc-500">
                        ...
                        <h4 className="font-medium text-zinc-300 mb-1">No Event Selected</h4>
                        <p className="text-xs">Select a webhook event from the table to view its delivery timeline and AI diagnosis.</p>
                    </div>
                ) : (
                    <div className="flex flex-col h-full overflow-y-auto">
                        <div className="px-5 py-4 border-b border-zinc-800/60 flex flex-col gap-1">
                            <span className="text-[10px] uppercase tracking-widest text-zinc-500 font-semibold">Inspector</span>
                            <h3 className="text-base font-medium text-zinc-100 font-mono">{selectedEventId}</h3>
                        </div>
                        
                        <div className="p-5 flex-1 flex flex-col gap-6">
                            {/* TIMELINE */}
                            <div>
                                <h4 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-4">Delivery Timeline</h4>
                                {!eventDetails ? (
                                    <div className="text-xs text-zinc-500">Loading timeline...</div>
                                ) : (
                                    <div className="relative pl-3 border-l border-zinc-800 space-y-6">
                                        {eventDetails.attempts.sort((a: any, b: any) => a.attempt_number - b.attempt_number).map((a: any, idx: number) => {
                                            const isSuccess = a.http_status >= 200 && a.http_status < 300;
                                            return (
                                                <div key={a.attempt_number} className="relative">
                                                    <div className={`absolute -left-[17px] top-1 w-2.5 h-2.5 rounded-full border-2 border-zinc-950 ${isSuccess ? 'bg-green-500' : 'bg-red-500'}`}></div>
                                                    <div className="flex flex-col gap-1">
                                                        <div className="flex items-center gap-2">
                                                            <span className="text-sm font-medium text-zinc-200">Attempt {a.attempt_number}</span>
                                                            <span className={`text-[10px] px-1.5 py-0.5 rounded font-mono ${isSuccess ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                                                                HTTP {a.http_status}
                                                            </span>
                                                        </div>
                                                        <span className="text-xs text-zinc-500">{a.timeout ? 'Connection timed out' : `Response in ${a.response_time_ms}ms`}</span>
                                                    </div>
                                                </div>
                                            );
                                        })}
                            ...
                            {/* INTELLIGENCE ML BOX */}
                            <div>
                                <h4 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-3">Model Analysis</h4>
                                {!intelligence ? (
                                    <div className="text-xs text-zinc-500 flex items-center gap-2"><div className="w-3 h-3 rounded-full border-2 border-zinc-500 border-t-transparent animate-spin"></div> Running inference...</div>
                                ) : (
                                    <div className="space-y-3">
                                        <div className="flex items-center justify-between p-4 rounded bg-zinc-900 border border-zinc-800 mb-3">
                                            <div className="flex flex-col">
                                                <span className="text-xs text-zinc-400">Replay Safety Score</span>
                                                <span className="text-sm font-medium text-zinc-100 mt-1">{intelligence.safe_to_replay ? 'Safe to Replay' : 'High Risk Event'}</span>
                                            </div>
                                            ...
                                            <div className="w-16 h-16 relative flex items-center justify-center">
                                                <svg className="absolute inset-0 w-full h-full -rotate-90">
                                                    ...
                                                    <circle 
                                                        ...
                                                        strokeDashoffset={175.9 - (175.9 * (intelligence.safe_to_replay ? 0.95 : 0.15))} 
                                                        className={`${intelligence.safe_to_replay ? 'text-green-500' : 'text-red-500'} transition-all duration-1000`} 
                                                    />
                                                </svg>
                                                <span className="text-sm font-bold text-zinc-100 relative z-10">{intelligence.safe_to_replay ? '95%' : '15%'}</span>
                                            </div>
                                        </div>
                                        
                                        <div className="flex items-center justify-between p-3 rounded bg-zinc-900 border border-zinc-800">
                                            <div className="flex flex-col">
                                                <span className="text-xs text-zinc-400">Behavioral Anomaly</span>
                                                <span className="text-sm font-medium text-zinc-100">{intelligence.is_anomaly ? 'Detected' : 'Normal Traffic'}</span>
                                            </div>
                                            <div className={`w-2 h-2 rounded-full ${intelligence.is_anomaly ? 'bg-red-500' : 'bg-green-500'}`}></div>
                                        </div>
                                        ...
                                        >
                                            {intelligence.safe_to_replay ? 'Execute Replay' : 'Replay Blocked'}
                                        </button>
                                    </div>
                                )}
                            </div>
