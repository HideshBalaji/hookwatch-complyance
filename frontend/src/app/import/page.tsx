...
import { Activity, Server, UploadCloud, LogOut, FileJson, FileText, Database, Loader2 } from 'lucide-react';
...
export default function ImportData() {
  ...
  const [activeTab, setActiveTab] = useState<'csv' | 'json_bulk' | 'json_single'>('csv');
  ...
  const handleCSVUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
                ...
                try {
                    ...
                    if (statusRes.ok) {
                        ...
                        if (statusData.status === 'processing') {
                            showToast({ message: `Ingesting logs... ${statusData.processed} / ${statusData.total} events`, type: "loading", taskId: taskId });
                        } else if (statusData.status === 'complete') {
                            clearInterval(interval);
                            setIsUploading(false);
                            showToast({ message: `Successfully ingested ${statusData.processed} events and ${statusData.attempts_processed} attempts.`, type: "success" });
                        } else if (statusData.status === 'failed') {
                            clearInterval(interval);
                            setIsUploading(false);
                            showToast({ message: `Upload failed: ${statusData.error}`, type: "error" });
                        }
                    }
                } catch(err) {
        ...
        setIsUploading(false);
        showToast({ message: e.message || 'Network error occurred.', type: "error" });
    }
  };
  ...
  const handleJsonSubmit = async () => {
    ...
    try {
        ...
        const data = await res.json();
        if(res.ok) {
            showToast({ message: data.message || 'Successfully ingested JSON data.', type: "success" });
            setJsonInput('');
        } else {
            showToast({ message: data.detail || 'Upload failed', type: "error" });
        }
    } catch(e: any) {
        showToast({ message: 'Invalid JSON format or network error.', type: "error" });
    } finally {
        setIsUploading(false);
    }
  };
    ...
    <div className="flex h-screen bg-zinc-950 text-zinc-300 font-sans text-sm selection:bg-zinc-800 selection:text-zinc-100 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-full bg-zinc-950 overflow-y-auto">
        <header className="px-6 py-4 flex items-center justify-between border-b border-zinc-800/60 sticky top-0 bg-zinc-950 z-10">
          <h2 className="text-lg font-medium text-zinc-100 tracking-tight">Data Ingestion</h2>
        </header>

        <div className="p-6 max-w-[1000px]">
          <div className="ui-panel p-6">
              <p className="text-zinc-400 mb-6">Select an ingestion method to manually upload webhook delivery logs into your workspace database.</p>
              ...
              <div className="flex border-b border-zinc-800 mb-6">
                  ...
                  >
                      <div className="flex items-center gap-2"><FileText className="w-4 h-4" /> Batch CSV Upload</div>
                  </button>
                  <button 
                  ...
                  >
                      <div className="flex items-center gap-2"><Database className="w-4 h-4" /> Bulk JSON Array</div>
                  </button>
                  <button 
                  ...
                  >
                      <div className="flex items-center gap-2"><FileJson className="w-4 h-4" /> Single JSON Event</div>
                  </button>
              </div>

              {activeTab === 'csv' && (
                  <div className={`flex flex-col items-center justify-center p-12 border rounded transition-colors ${isUploading ? 'border-zinc-600 bg-zinc-900' : 'border-dashed border-zinc-700 bg-zinc-900/50'}`}>
                      {isUploading ? (
                      ...
                      <h3 className="text-zinc-200 font-medium mb-1">{isUploading ? 'Uploading...' : 'Upload CSV Logs'}</h3>
                      <p className="text-zinc-500 text-xs mb-6 max-w-sm text-center">
                          {isUploading ? 'Please wait while we parse and ingest the data into the database. You can navigate away from this page safely.' : 'Upload a CSV file containing columns for event_id, event_type, created_at, endpoint_id, attempt_number, http_status, response_time_ms, and timeout.'}
                      </p>
                  ...
                  <div className="flex flex-col gap-4">
                      <div>
                          <label className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2">
                              {activeTab === 'json_bulk' ? 'Paste JSON Array' : 'Paste JSON Object'}
                          </label>
                          <textarea 
                              ...
                              placeholder={activeTab === 'json_bulk' ? '[\n  {\n    "event_id": "...",\n    "event_type": "...",\n    ...\n  }\n]' : '{\n  "event_id": "...",\n  "event_type": "...",\n  ...\n}'}
                              value={jsonInput}
